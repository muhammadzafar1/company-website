              ...
              <motion.article key={project.title || index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.5, delay: index * 0.08 }} className="group overflow-hidden rounded-[1.75rem] border border-white/10 bg-slate-900/70">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-6 flex gap-3">
                    {project.liveDemo && <a href={project.liveDemo} className="inline-flex items-center rounded-full bg-gradient-to-r from-brand to-brand-dark px-4 py-2 text-sm font-medium text-white">Live Demo</a>}
                    {project.github && <a href={project.github} className="inline-flex items-center rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white">GitHub</a>}
                  </div>
                </div>
              </motion.article>
            ))}
